...
public class ProductService {
    ...
    public Optional<Product> getProductInventoryById(String id) throws JsonProcessingException {
        ...
        try {
            ResponseEntity<String> catalogResponseEntity =
                    restTemplate.getForEntity("http://catalog-service:8181/api/catalog/{id}",
                            String.class,
                            id);
            ...
            if (catalogResponseEntity.getStatusCode() == HttpStatus.OK) {
                ...
                ResponseEntity<String> inventoryResponseEntity = restTemplate.getForEntity("http://inventory-service:8282/api/inventory/{id}", String.class, id);
                ...
                log.error("Unable to get inventory level for id: " + id + ", StatusCode: " + catalogResponseEntity.getStatusCode());
    ...
    @HystrixCommand(fallbackMethod = "getDefaultProductsInventoryBySku")
    public Optional<List<Product>> getProductsInventoryBySku(String sku) throws JsonProcessingException {
        ObjectMapper objectMapper = new ObjectMapper();

        log.info("Sending request to catalog...");
        try {
            ResponseEntity<String> catalogResponseEntity = restTemplate.getForEntity("http://catalog-service:8181/api/catalog/range/" + sku, String.class);
            log.info("Response from catalog: " + catalogResponseEntity.getBody());

            String availableIds = Objects.requireNonNull(catalogResponseEntity.getBody()).replaceAll("[\\[\\]\"]", "");

            if (catalogResponseEntity.getStatusCode() == HttpStatus.OK) {
                log.info("Sending request to inventory...");
                ResponseEntity<String> inventoryResponseEntity = restTemplate.getForEntity("http://inventory-service:8282/api/inventory?uniqIds=" + availableIds, String.class);
                log.info("Response from inventory: " + inventoryResponseEntity.getBody());

                return Optional.of(Arrays.stream(objectMapper.readValue(inventoryResponseEntity.getBody(), Product[].class))
                        .sequential()
                        .filter(obj -> obj.getAvailableQuantity() > 0)
                        .collect(Collectors.toList()));
            } else {
                log.error("Unable to get inventory level for sku: " + sku + ", StatusCode: " + catalogResponseEntity.getStatusCode());
                return Optional.empty();
            }
        } catch (HttpClientErrorException.NotFound e) {
